...
async def cmd_backup(message: Message) -> None:
    ...
    ch = len((await persist.snapshot()).get("channels") or [])
    try:
        await message.answer_document(
            BufferedInputFile(data, filename=name),
            caption=(
                "🛡 <b>ZAXIRA (akkaunt + kanallar)</b>\n"
                f"Kanallar: <b>{ch}</b> ta\n"
                "<i>Bu faylni saqlab qo'ying. Yangilanishdan keyin akkaunt yoki "
                "kanallar yo'qolsa — shu faylni botga yuboring, hammasi qaytadi.</i>"
            ),
            parse_mode="HTML",
        )
